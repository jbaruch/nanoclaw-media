"""Tests for skills/youtube-comment-check/scripts/stamp-cursor.py."""
...
SCRIPT_REL = "skills/youtube-comment-check/scripts/stamp-cursor.py"
...
def test_stamp_leaves_no_tempfile_debris(stamp_module, tmp_path):
    ...
    now = datetime(2026, 5, 2, 3, 0, tzinfo=timezone.utc)
    stamp_module.stamp(cursor, now)
    leftover = list(tmp_path.glob(".cursor.json.*.tmp"))
    assert leftover == []
...
def test_stamp_preserves_existing_file_mode(stamp_module, tmp_path):
    ...
    now = datetime(2026, 5, 2, 3, 0, tzinfo=timezone.utc)
    stamp_module.stamp(cursor, now)
    after_mode = stat.S_IMODE(os.stat(cursor).st_mode)
...
def test_main_cli_flag_overrides_env_var(tmp_path):
    ...
    env = {**os.environ, "YOUTUBE_COMMENT_CHECK_CURSOR": str(env_cursor)}
    proc = subprocess.run(
...
def test_main_exits_2_on_write_failure(tmp_path):
    ...
    try:
        proc = subprocess.run(
            ["python3", str(REPO_ROOT / SCRIPT_REL), "--cursor", str(cursor)],
            capture_output=True,
            text=True,
            timeout=10,
        )
