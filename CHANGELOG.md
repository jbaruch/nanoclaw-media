...
The last round covered the two remaining write paths. A send that succeeds while the follow-up write fails would leave `notified` false and re-alert on the next fire, so Step 4 now surfaces that error naming every title already delivered, states that their flags did not persist, and stops rather than adding more unrecorded sends — a bounded, visible duplicate instead of a silent one. Deleting the stamp on a *failed* send matters for the same reason from the other direction: a Step 3 release carries `last_verdict: unknown`, which the backoff would sit on for up to a month with the alert never delivered. And `recommend-shows` Step 9, the watchlist's other writer, now handles `schema_version` explicitly — stamps `1` on a record it creates or appends to unstamped, preserves `1`, and writes nothing at any other version rather than appending into a shape it cannot interpret.
