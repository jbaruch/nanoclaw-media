...
| `recommend-shows` (`scripts/append-watchlist.py`, run by Step 9; Step 6 reader) | non-owner writer | Appends `tracking` entries (`title`, `platform`, `expected`, `reason`, `added`, each a required non-empty string) and writes `notified: false` itself, only to a record already at integer version `1`; stamps `1` only on a record it creates when the file is absent. An unstamped, otherwise-versioned, empty, or malformed record is read-only to it — it never migrates and never rewrites another skill's fields. Refuses an `expected` the precheck cannot anchor |
